...
plot_cond_eff(rtage_fit, xvar = 'age', xname = 'Age', yname = 'Reaction Time')
...
print(plot_cond_eff(fit = rtage_fit, xvar = 'age', xname = 'Age', yname = 'Reaction Time',
        ...
        geom_hex(data = rtage_fit$data,
                 binwidth = c(.5, .05), alpha = .5,
                 aes(x = age, y = RT.shape,
                     fill = log(..count..))))
```
## Model summary
...
library(bayesplot)
library(data.table)
rtageprepot_fit <- read_fit_models('rtageprepotalt_old')
rtageprepot_null_fit <- read_fit_models('rtageprepotnull')

print(plot_cond_eff(fit = rtageprepot_fit, xvar = 'age', xname = 'Age', yname = 'Reaction Time',
                    conditions = data.frame(exact_prepotency_ofactor = 0:3), 
                    condition_title = 'N prev "Go" trials', mean_center_y = FALSE,
                    method = 'posterior_epred', points = TRUE))
...
```{r eval=FALSE}
rtageprepot_newdata_by_cols <- names(rtageprepot_fit$data)
rtageprepot_newdata_by_cols <- rtageprepot_newdata_by_cols[rtageprepot_newdata_by_cols != 'RT.shape']
rtageprepot_newdata <- as.data.table(rtageprepot_fit$data)[, .(y_med = median(RT.shape),
                                                               y_upper = quantile(RT.shape, .8),
                                                               y_lower = quantile(RT.shape, .2)),
                                                           by = rtageprepot_newdata_by_cols]
rtageprepot_yrep_ <- as.data.table(posterior_summary(posterior_epred(rtageprepot_fit, 
                                                                     newdata = rtageprepot_newdata)))
rtageprepot_yrep <- cbind(rtageprepot_yrep_, rtageprepot_newdata)
ggplot(rtageprepot_yrep, 
       aes(x = y_med, xmin = y_lower, xmax = y_upper,
           y = Estimate, ymin = Q2.5, ymax = Q97.5,
           color = age)) + 
  geom_errorbar(aes(fill = NULL, color = NULL), alpha = .15) +
  geom_errorbarh(aes(fill = NULL, color = NULL), alpha = .15) +
  # geom_rect(aes(fill = NULL, color = NULL), alpha = .15) + 
  geom_point(size = .5) + 
  geom_abline(intercept = 0, slope = 1) + 
  facet_grid(~ exact_prepotency_ofactor) + 
  theme_minimal() + 
  theme(panel.grid = element_blank())


plot(conditional_effects(rtageprepot_fit), ask = FALSE)
```

## Model summary

```{r}
summary(rtagepropot_fit)
```

## Posterior predictive check

```{r}
pp_check(rtagepropot_fit) + theme_minimal()
```
