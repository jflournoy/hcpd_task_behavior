...
Number of participants with dowloaded task MRI, but no CARIT task behavior: `r staged_dlmri[has_task_scan == TRUE][!d, on = 'sID'][, .N]`.

Number of participants with CARIT task behavior, but no dowloaded task MRI (on this task): `r unique(d[, 'sID'])[!staged_dlmri[has_task_scan == TRUE], on = 'sID'][, .N]`. This indicates that these participants had problems with their data in the multirun ica fix pipeline (it is possible that this session will eventually get rebuilt, it seems).
...
ggplot(accuracy_summary, aes(x = age, y = mean_accuracy)) + 
  ...
  geom_line(stat = 'smooth', method = 'gam', color = apal[[2]], size = .5, alpha = 1) +
  geom_point(data = accuracy_summary[(trialType == 'go' & mean_accuracy == 0) | 
                                       (trialType == 'nogo' & mean_accuracy == 1)],
             color = apal[[4]], size = 1,
             position = position_jitter(w = .125, h = .025)) +
