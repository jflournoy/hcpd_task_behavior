...
Number of participants with task behavior not staged, recovered from intake project: `r length(unique(d_intake$sID))`

Number of intake participants with no demo info: `r d_intake[!demos, on = 'sID'][, list(sID = unique(sID))][, .N]`. Participant ids: `r d_intake[!demos, on = 'sID'][, list(sID = unique(sID))]`.

Number of participants not in either intake or staged CARIT behavior lists: `r demos[!d, on = 'sID'][!d_intake, on = 'sID'][, .N]`.

List of those:

`r demos[!d, on = 'sID'][!d_intake, on = 'sID'][, sID]`
...
Number of PR participants: `r public_release[, .N]`

Number of PR participants with CARIT behavior: `r unique(d_combined[public_release, on = 'sID'][!is.na(PR), 'sID'])[, .N]`

Number of PR participants with CARIT task scan: `r staged_dlmri[public_release, on = 'sID'][has_carit == TRUE][, .N]`
...
rt_repeated_go_trialnum_fit <- lmer(
  RT.shape_s ~ 1 + N_of_trialType_s*trialNum_s*runN_fac + (1 + N_of_trialType_s + runN_fac | sID), 
...
if(!file.exists(acc_mod_expanded.fn)){
  acc_mod_expanded <- glmer(is_FA ~ 1 + prepotency*runN_fac*trialNum_s + (1 | sID),
                            data = carit_FA_frame, family = 'binomial')
  saveRDS(acc_mod_expanded, file = 'acc_mod_expanded.RDS')
} else {
  acc_mod_expanded <- readRDS(file = 'acc_mod_expanded.RDS')
}
